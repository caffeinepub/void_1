...
export async function encryptBytes(bytes: Uint8Array, key: CryptoKey): Promise<string> {
  ...
  const cipherBuf = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, key, plainArrayBuffer);
  const combined = new Uint8Array(iv.length + cipherBuf.byteLength);
  combined.set(iv, 0);
  combined.set(new Uint8Array(cipherBuf), iv.length);
  return btoa(String.fromCharCode(...combined));
...
export async function decryptBytes(ciphertext: string, key: CryptoKey): Promise<Uint8Array | null> {
  try {
    const combined = Uint8Array.from(atob(ciphertext), (c) => c.charCodeAt(0));
    const iv = combined.slice(0, 12);
    const data = combined.slice(12);
